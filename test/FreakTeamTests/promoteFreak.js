...
describe("Promote Freak -- Tests", function () {
  ...
  it("PromoteFreak fails when it doesn t emit an event ", async function () {
    await expect(freak.connect(hr).promoteFreak(other.address, 2))
      .to.emit(freak, "promotedFreak")
      .withArgs(1, 1, 2);
  });
});
