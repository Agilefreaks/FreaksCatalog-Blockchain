...
describe("Change Freak Norm -- Tests", function () {
  ...
  it("ChangeFreakNorm fails when it's not called by hr", async function () {
    await expect(freak.connect(other).changeFreakNorm(other.address, 1)).to.be.revertedWith('Caller is not a hr');
  });

  it("ChangeFreakNorm fails when caller try to promote himself", async function () {
    await expect(freak.connect(hr).changeFreakNorm(hr.address, 1)).to.be.revertedWith('Caller can not promote himself');
  });
  ...
  it("ChangeFreakNorm fails when it doesn t emit an event ", async function () {
    await(expect( freak.connect(hr).changeFreakNorm(other.address, 1)).to.emit(freak, "changedFreakNorm").withArgs(1, 0, 1));
  });
