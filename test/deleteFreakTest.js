...
describe("Delete Freak -- Tests", function () {
  ...
  it("DeleteFreak fails when the delete function does not delete everything", async function () {
    await freak.connect(hr).deleteFreak(other.address);
    expect(await freak.balanceOf(other.address, (await freak.freaks(other.address)).employeeNumber)).to.equal(0);
  });
  ...
  it("DeleteFreak fails when it doesn t emit an event", async function () {
    await(expect( freak.connect(hr).deleteFreak(other.address)).to.emit(freak, "deletedFreak").withArgs(other.address, 1));
  });
