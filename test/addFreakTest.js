...
describe("Add Freak -- Tests", function () {
  ...
  it("AddFreak fails when it's not called by hr", async function () {
    await expect(freak.connect(other).addNewFreak(other.address, "TestFreak", 2021, 1, 1, 1, 0)).to.be.revertedWith('Caller is not a hr')
  });

  it("AddFreak fails when the name is not added", async function () {
    await freak.connect(hr).addNewFreak(other.address, "TestFreak", 2021, 1, 1, 1, 0);
  ...
  it("AddFreak fails when role is not added", async function () {
    await freak.connect(hr).addNewFreak(other.address, "TestFreak", 2021, 1, 0, 1, 0);
    expect((await freak.freaks(other.address)).role).to.equal(0)
  });

  it("AddFreak fails when skill is not added", async function () {
    await freak.connect(hr).addNewFreak(other.address, "TestFreak", 2021, 1, 1, 2, 0);
  ...
  it("AddFreak fails when it doesn t emit an event", async function () {
    await(expect( freak.connect(hr).addNewFreak(other.address, "TestFreak", 2021, 1, 1, 1, 0)).to.emit(freak, "addedFreak").withArgs(other.address, "TestFreak", 2021, 1, 1, 1, 0, 93));
  });
});